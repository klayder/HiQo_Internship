...
class Escape {
  ...
  checkPass(posX, posY) {
    ...
    return this.labyrinth[posY][posX] === 0;
  }
  ...
  choseDirection() {
    this.branchesCont = 0;
    for (const dir in this.availableWays) {
  ...
  goBack() {
    ...
    this.currentPosition = breadСrumb.position.split(':').map(item => +item);
  }
